...
def get_args_parser():
    ...
    parser.add_argument('--patient_dataset_type', default='Center2D', type=str, choices=['3D', 'Center2D', 'Center2D_flash_attn',  '3D_flash_attn', '3D_st', '3D_st_joint', '3D_st_flash_attn', '3D_st_joint_flash_attn', '3D_st_flash_attn_nodrop'], help='patient dataset type')
    ...
    parser.add_argument('--task_mode', default='binary_cls', type=str, choices=['binary_cls', 'multi_cls', 'multi_label', 'multi_task', 'multi_task_default'], help='Task mode for the dataset')
...
def main(args):
    ...
    if args.k_fold and args.patient_dataset: # K-fold cross validation, if your dataset is large, we recommend to use single_fold and run multiple times to avoid CUDA failure
        ...
        assert args.single_fold and args.split_path is not None
        fold_results = []
        fold_results_test = []
        ...
        if args.linear_probe:
            ...
            if args.patient_dataset_type == '3D' or args.patient_dataset_type == '3D_flash_attn' or args.patient_dataset_type == '3D_st_flash_attn_nodrop':
                for _, p in model.fc_norm.named_parameters():
                    p.requires_grad = True
                for _, p in model.aggregate_cls_norm.named_parameters():
                    p.requires_grad = True
                for _, p in model.fc_aggregate_cls.named_parameters():
                    p.requires_grad = True
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if max_flag is True:
                print(f"Max AUC: {max_auc}, Max ACC: {max_accuracy}, Max AUCPR: {max_auc_pr}, Max Bal Acc: {max_bal_acc}, at epoch {epoch}")
                print(f"Max AUC: {max_auc}, Max ACC: {max_accuracy}, Max AUCPR: {max_auc_pr}, Max Bal Acc: {max_bal_acc}, at epoch {epoch}", file=open(os.path.join(args.output_dir, f"auc_singlefold.txt"), mode="a"))
            ...
            if max_flag or epoch == (args.epochs - 1) or args.always_test:
                ...
                if max_flag_test is True:
                    print(f"Max AUC: {max_auc_test}, Max ACC: {max_accuracy_test}, Max AUCPR: {max_auc_pr_test}, Max Bal Acc: {max_bal_acc_test}, at epoch {epoch}")
                    print(f"Max AUC: {max_auc_test}, Max ACC: {max_accuracy_test}, Max AUCPR: {max_auc_pr_test}, Max Bal Acc: {max_bal_acc_test}, at epoch {epoch}", file=open(os.path.join(args.output_dir, f"auc_test_singlefold.txt"), mode="a"))

                # storing the test metrics for the best val epoch
                cur_auc_test = test_auc_roc
                cur_auc_pr_test = test_auc_pr
                cur_acc_test = test_stats['acc1']
        ...
        print(f"Fold results: {fold_results_test_for_best_val}\nMean: {fold_results_mean_test_for_best_val}\nStd: {fold_results_std_test_for_best_val}")
        print(f"Fold results: {fold_results_test_for_best_val}\nMean: {fold_results_mean_test_for_best_val}\nStd: {fold_results_std_test_for_best_val}",
            file=open(os.path.join(args.output_dir, "fold_results_test_for_best_val.txt"), mode="a"))
